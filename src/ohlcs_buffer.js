...
/**
 * @classdesc OhlcsBuffer
 * 
 * Ohlcs = Open, High, Low, Close, Spread
 * windowSize間隔内の最大Spreadを保持する
 */
class OhlcsBuffer {
     ...
     * @constructor
     *
     * @param windowSize string   OHLCSの間隔 (1m, 10h,...)
     */
    ...
    addPrice(price, spread, timestamp) {
        ...
        if (lastOhlc && lastOhlc.ts == normalizedTs) {
            ...
            if (lastOhlc.spread < spread) {
                lastOhlc.spread = spread
            }
        } else {
            this.list.push({
