...
/**
 * @classdesc PriceBuffer
 * 
 * WindowSizeで指定された各時間間隔の最終価格を保持するバッファ.
 * チャートに表示するわけではないためOHLCは必要なし.
 */
class PriceBuffer {
    ...
    addPrice(price, timestamp) {
        ...
        let lastBuffer = this.getLast()
        if (lastBuffer && lastBuffer.ts == normalizedTs) {
            lastBuffer.price = price;
        } else {
            this.list.push({
